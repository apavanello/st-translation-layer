...
/**
 * @param {() => {baseUrl: string, apiKey: string, model: string, language: string}} getConfig
 */
export function createTranslationService(getConfig) {
    async function callOnce(messages, maxTokens, signalParent) {
        const { baseUrl, apiKey, model } = getConfig();
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), TIMEOUT_MS);
        signalParent?.addEventListener('abort', () => controller.abort(), { once: true });
        try {
            ...
            if (!content) {
                throw new TranslationError('bad_output', 'empty completion');
            }
